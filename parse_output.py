...
elastic_constant = 10E20 #elastic constant of CaTiO3 P4bm
strain_step = 0.008
min_force = 0.00001
min_strain = 0.000001
...
def make_coo_start():
	...
	new_col=np.ones((40,1),dtype='int')
	coo_start_new_cart_cord=np.append(new_cart_cord,new_col,1)
	file_coo_start=open("POSCAR", 'w+')
	file_coo_start.write('poscar from lautrec output using parse_output.py\n')
	file_coo_start.write('    1.0 \n')
	np.savetxt(file_coo_start,prim_mat,fmt='%.8f',delimiter='\t')
	file_coo_start.write('\n\nDirect\n')
	np.savetxt(file_coo_start,frac_cord,fmt='%2.10f          %2.10f        %2.10f',delimiter='\t')
